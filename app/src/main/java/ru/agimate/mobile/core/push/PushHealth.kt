...
enum class PushHealth {
    ...
    /**
     * Часть каналов зарегистрирована, часть — нет. С одним транспортом такого состояния не было, а
     * с двумя оно штатное: RuStore переподписывается через `ru.vk.store` и на это время исчезает,
     * FCM продолжает доставлять. Чинить надо — уведомления при этом **приходят**, и говорить
     * человеку обратное значит врать ему о работающей шторке.
     */
    Partial,
    ...
    /** Все «нет» лечатся одинаково — перерегистрацией, и она идемпотентна. */
    val fixable: Boolean get() = this == Missing || this == Stale || this == Partial

    /** Дойдёт ли уведомление прямо сейчас: хватает одного живого канала. */
    val delivering: Boolean get() = this == Working || this == Partial
}
 ...
 *
 * Спека требует совпадения хотя бы одной записи. Для **починки** берём строже — чтобы своя нашлась
 * у каждого поднятого транспорта: лишняя перерегистрация ничего не стоит и идемпотентна, а
 * непойманное расхождение стоит уведомлений. Но для **человека** строгость неуместна: пока хоть
 * один канал зарегистрирован, уведомления доходят, и это [PushHealth.Partial], а не поломка.
 * Лишние записи на сервере не мешают: их там штатно две-три, пока не отвалится старый токен.
 */
fun pushHealth(remote: List<PushSubscriptionDto>, local: Map<String, String>): PushHealth = when {
    ...
    local.any { (provider, token) -> remote.any { it.holds(provider, token) } } -> PushHealth.Partial
