 ...
 *
 * Пакет назван по коннектору, а не по ресурсу, и имя выглядит устаревшим сильнее, чем оно есть:
 * `webchat` — живое значение `connectorCode`, которым сервер отличает эти переписки от
 * мессенджеров и IDE, и половина путей ниже по-прежнему с него начинается. Переименовать пакет в
 * `sessions` значит разойтись и с адресами внутри, и с содержимым: здесь ещё контакты и токены.
 *
