...
class FileNamesTest {
    ...
    fun `an ordinary name is left alone`() {
        assertEquals("отчёт.pdf", diskFileName("отчёт.pdf", "pdf"))
    }

    /** Регрессия: иначе «сохранить» пишет файл куда угодно, только не в свою папку. */
    @Test
    fun `a path in the name is cut down to the name`() {
        assertEquals("passwd", diskFileName("../../etc/passwd", null))
        assertEquals("файл.txt", diskFileName("""C:\Windows\файл.txt""", null))
    }

    @Test
    fun `characters the file system refuses become underscores`() {
        assertEquals("счёт_2026_05.pdf", diskFileName("счёт:2026|05.pdf", null))
    }

    @Test
    fun `a name without an extension takes one from the mime type`() {
        assertEquals("снимок.jpg", diskFileName("снимок", "jpg"))
    }

    /** Сервер знает про файл больше, чем его mime: своё расширение главнее выведенного. */
    @Test
    fun `the name keeps its own extension`() {
        assertEquals("данные.csv", diskFileName("данные.csv", "txt"))
    }

    @Test
    fun `a nameless attachment still gets a name`() {
        assertEquals("файл.png", diskFileName(null, "png"))
        assertEquals("файл", diskFileName("   ", null))
    }
    ...
    fun `a leading dot does not make the file hidden`() {
        assertEquals("gitignore", diskFileName(".gitignore", null))
    }

    @Test
    fun `a very long name is trimmed but keeps its extension`() {
        val saved = diskFileName("и".repeat(300) + ".pdf", null)
